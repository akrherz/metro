...
*     ---------------------------------------
      G(0) = 0.0
*      WRITE(*,*) "now, next", now, next
*      WRITE(*,*) "iref", iref
      do j=1,iref
         T(j,now)=ITP(j)
         T(j,next)=ITP(j)
*         WRITE(*,*) "T(j, now)", j, now, T(j, now) 
         G(j) = 0.
      end do
*      WRITE(*,*) 'ITP(1)', ITP(1)
*      WRITE(*,*) "T(1, now)", 1, now, T(1, now) 
*      WRITE(*,*) "T(1, next)", 1, next, T(1, next) 
*      WRITE(*,*) "---------------------------"
*     ++++++++++++++++++++++++++++++++++
*      Entree de la boucle de prevision
*     ++++++++++++++++++++++++++++++++++
*      WRITE(*,*) "NTP, NTFM",NTP,NTFM
      do i = NTP+1,NTFM
*         WRITE(*,*) "i",i
...
*         WRITE(*,*) "TA", i, TA(i) 
*         WRITE(*,*) "1,T (1, next)", i, T(1, next) 
*         WRITE(*,*) "1, T(1, now)", i, T(1, now) 
         TSK = T(1,now) + TCDK
...
*         WRITE(*,*) "P0(i)",P0(i)
*         WRITE(*,*) "RGASD",RGASD
*         WRITE(*,*) "FOTVT ( TSK , QG )",FOTVT ( TSK , QG )
*         WRITE(*,*) "TSK, QG", TSK, QG
...
*        +++++++++++++++++++++++++++
*         WRITE(*,*) "2, T(1, next)", 1, next, T(1, next) 
...
*         WRITE(*,*) "CPD*(TA(i)-T(1,now))",CPD*(TA(i)-T(1,now))
*         WRITE(*,*) RHO*CTU*( CPD*(TA(i)-T(1,now)) + CL*(QA(i)-QG))
*         WRITE(*,*) PRG
**         WRITE(*,*) "COFS",COFS
**         WRITE(*,*) "AL",AL
**         WRITE(*,*) "FS(i)",FS(i)
...
*        conservation d'energie
         if ( FGD.lt.0.0 .and. G(0).gt.0.0) then
            ...
            DX = 0.0
*            WRITE(*,*) "T(1, now)", i,T(1, now) 
            T(1,next) = T(1,now)+DT*(CNT(1,2)*( G(1) - G(0)))
*            WRITE(*,*) "T(1, next)", i, T(1, next), G(1), G(0)
         end if
...
*         WRITE(*,*) "T(1, next)", i, next, T(1, next)
