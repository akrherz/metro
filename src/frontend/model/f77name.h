...
#if defined(HP) || defined (SUN) || defined (linux)
...
#define swap_word_endianness(mot) { register unsigned long tmp =(unsigned long)mot; \
   mot = (tmp>>24) | (tmp<<24) | ((tmp>>8)&0xFF00) | ((tmp&0xFF00)<<8); }
#define swap_buffer_endianness(buff,nwds) {\
    unsigned long *buf=(unsigned long *)buff ;\
   long nwords=nwds ;\
