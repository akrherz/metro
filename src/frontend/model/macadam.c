...
** Date:        April 16, 2004
**
** Description: Fichier qui gère le modèle de METRo (macadam) lui-même.
...
/*  if(!bSilent){
    ...
    printf("dLCorr:%0.1f\n", dLCorr);   
    printf("Type (%d zone)\n",nNbrOfZone);   
    printf("nNbrTimeSteps %d\n", nNbrTimeSteps); 
    printf("dDeltaT: %0.2f\n",dDeltaT); 
    printf("nLenObservation: %d\n",nLenObservation); 
    
    for(i=0; i<nNbrOfZone; i++){ 
      printf(" Layer type: %d\n", npMateriau[i]); 
      printf(" Layer thickness: %0.2f\n", dpZones[i]); 
    } 
    printf("[[ Mode manuel de prevision ]]\n"); 
  }
...
*/  nLenObservation = nLenObservation -1;
  ...
  bFail = FALSE;
  if(bpNoObs[2]){
    /* Pas d'observation valide */
//    if(!bSilent)
//      printf("Observations non-disponible.\n");
    goto liberation;
  }
  else if(bpNoObs[3]){
    BOOL bFalse = FALSE;
